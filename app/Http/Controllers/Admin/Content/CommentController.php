...
class CommentController extends Controller
{
    ...
    {
        $unseenComments = Comment::where('commentable_type', '=', 'App\Models\Content\Post')->where('seen', '=', 0)->cursor();
        foreach ($unseenComments as $unseenComment) {
            $unseenComment->seen = 1;
            $unseenComment->save();
        }
        $comments = Comment::where('commentable_type', '=', 'App\Models\Content\Post')->orderByDesc('created_at')->cursorPaginate(10);
    ...
    public function answer(CommentRequest $request, Comment $comment)
    {
        if ($comment->parent_id === null) {
            ...
            return redirect()->route('admin.content.comments.index')->with('toastr-success', 'پاسخ شما با موفقیت ثبت شد');
        } else {
