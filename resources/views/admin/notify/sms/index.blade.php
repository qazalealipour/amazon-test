...
@section('title', 'لیست اطلاعیه های پیامکی')

@section('page-title', 'لیست اطلاعیه های پیامکی')

@push('breadcrumb')
    <li class="breadcrumb-item">
        <span class="bullet bg-gray-300 w-5px h-2px"></span>
    </li>
    <li class="breadcrumb-item text-dark">اطلاعیه های پیامکی</li>
@endpush
    ...
    <div class="card card-flush">
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                <a href="{{ route('admin.notify.sms.create') }}" class="btn btn-primary">افزودن اطلاعیه پیامکی</a>
            </div>
        </div>
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_sales_table">
                        <thead>
                            <tr class="text-center text-gray-400 fw-bolder fs-6 text-uppercase gs-0">
                                ...
                                <th class="min-w-200px">عنوان پیامک</th>
                                <th class="min-w-400px">متن پیامک</th>
                        ...
                        <tbody class="fw-bold text-gray-600 text-center">
                            @foreach ($sms as $key => $item)
                                <tr>
                                    ...
                                    <td>
                                        <span class="fw-bold text-hover-primary text-gray-800">{{ $item->title }}</span>
                                    </td>
                                    <td>
                                        <span class="fw-bold text-hover-primary text-gray-800">{{ Illuminate\Support\Str::limit($item->body, 40) }}</span>
                                    </td>
                                    <td>{{ jalaliDate($item->published_at) }}</td>
                                    <td>
                                        <div class="form-check form-check-solid form-switch ms-7">
                                            <input type="checkbox" name="status" id="{{ $item->id }}" class="form-check-input w-35px h-20px" onchange="changeStatus({{ $item->id }})" data-url="{{ route('admin.notify.sms.change-status', [$item->id]) }}" @checked($item->status === 1)>
                                        </div>
                                    </td>
                                    <td>
                                        <a href="{{ route('admin.notify.sms.edit', [$item->id]) }}" class="btn btn-icon btn-bg-light btn-active-light-primary w-30px h-30px me-3">
                                        ...
                                        <form action="{{ route('admin.notify.sms.destroy', [$item->id]) }}" method="post" class="d-inline">
                                            @csrf
                                            @method('delete')
                ...
                success: function(response) {
                    if (response.status) {
                        if (response.checked) {
                            ...
                            successToastr('اطلاعیه پیامکی با موفقیت فعال شد');
                        } else {
                            element.prop('checked', false);
                            successToastr('اطلاعیه پیامکی با موفقیت غیر فعال شد');
                        }
                    } else {
