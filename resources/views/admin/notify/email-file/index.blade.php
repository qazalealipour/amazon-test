...
@section('title', 'لیست فایل های ضمیمه شده')

@section('page-title', 'لیست فایل های ضمیمه شده')
...
@push('breadcrumb')
    ...
    <li class="breadcrumb-item text-dark">فایل های ضمیمه شده</li>
@endpush
    ...
    <div class="card card-flush">
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                <a href="{{ route('admin.notify.email-files.create', [$email->id]) }}" class="btn btn-primary">افزودن فایل</a>
            </div>
        </div>
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_sales_table">
                        <thead>
                            <tr class="text-center text-gray-400 fw-bolder fs-6 text-uppercase gs-0">
                                ...
                                <th class="min-w-300px">عنوان ایمیل</th>
                                <th class="min-w-100px">سایز فایل</th>
                                <th class="min-w-100px">نوع فایل</th>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="form-check form-check-solid form-switch ms-7">
                                            <input type="checkbox" name="status" id="{{ $file->id }}" class="form-check-input w-35px h-20px" onchange="changeStatus({{ $file->id }})" data-url="{{ route('admin.notify.email-files.change-status', [$file->id]) }}" @checked($file->status === 1)>
                                        </div>
                                    </td>
                                    <td>
                                        <a href="{{ route('admin.notify.email-files.edit', [$file->id]) }}" class="btn btn-icon btn-bg-light btn-active-light-primary w-30px h-30px me-3">
                                        ...
                                        <form action="{{ route('admin.notify.email-files.destroy', [$file->id]) }}" method="post" class="d-inline">
                                            @csrf
                                            @method('delete')
                ...
                success: function(response) {
                    if (response.status) {
                        if (response.checked) {
                            ...
                            successToastr('فایل مورد نظر با موفقیت فعال شد');
                        } else {
                            element.prop('checked', false);
                            successToastr('فایل مورد نظر با موفقیت غیر فعال شد');
                        }
                    } else {
