...
@section('title', 'لیست اطلاعیه های ایمیلی')

@section('page-title', 'لیست اطلاعیه های ایمیلی')

@push('breadcrumb')
    <li class="breadcrumb-item">
        <span class="bullet bg-gray-300 w-5px h-2px"></span>
    </li>
    <li class="breadcrumb-item text-dark">اطلاعیه های ایمیلی</li>
@endpush
    ...
    <div class="card card-flush">
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                <a href="{{ route('admin.notify.email.create') }}" class="btn btn-primary">افزودن اطلاعیه ایمیلی</a>
            </div>
        </div>
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_sales_table">
                        <thead>
                            <tr class="text-center text-gray-400 fw-bolder fs-6 text-uppercase gs-0">
                                ...
                                <th class="min-w-200px">موضوع ایمیل</th>
                                <th class="min-w-400px">متن ایمیل</th>
                                <th class="min-w-225px">تاریخ ارسال</th>
                        ...
                        <tbody class="fw-bold text-gray-600 text-center">
                            @foreach ($emails as $key => $email)
                                <tr>
                                    ...
                                    <td>
                                        <span class="fw-bold text-hover-primary text-gray-800">{{ $email->subject }}</span>
                                    </td>
                                    <td>
                                        <span class="fw-bold text-hover-primary text-gray-800">{{ Illuminate\Support\Str::limit($email->body, 40) }}</span>
                                    </td>
                                    <td>{{ jalaliDate($email->published_at) }}</td>
                                    <td>
                                        <div class="form-check form-check-solid form-switch ms-7">
                                            <input type="checkbox" name="status" id="{{ $email->id }}" class="form-check-input w-35px h-20px" onchange="changeStatus({{ $email->id }})" data-url="{{ route('admin.notify.email.change-status', [$email->id]) }}" @checked($email->status === 1)>
                                        </div>
                                    </td>
                                    <td>
                                        <a href="{{ route('admin.notify.email-files.index', [$email->id]) }}" class="btn btn-icon btn-bg-light btn-active-light-primary w-30px h-30px me-3">
                                            <span data-bs-toggle="tooltip" data-bs-trigger="hover" title="" data-bs-original-title="فایل های ضمیمه شده">
                                        ...
                                        <a href="{{ route('admin.notify.email.edit', [$email->id]) }}" class="btn btn-icon btn-bg-light btn-active-light-primary w-30px h-30px me-3">
                ...
                success: function(response) {
                    if (response.status) {
                        if (response.checked) {
                            ...
                            successToastr('اطلاعیه ایمیلی با موفقیت فعال شد');
                        } else {
                            element.prop('checked', false);
                            successToastr('اطلاعیه ایمیلی با موفقیت غیر فعال شد');
                        }
                    } else {
