...
@section('title', 'لیست اولویت تیکت ها')

@section('page-title', 'لیست اولویت تیکت ها')

@push('breadcrumb')
    <li class="breadcrumb-item">
        <span class="bullet bg-gray-300 w-5px h-2px"></span>
    </li>
    <li class="breadcrumb-item text-dark">اولویت تیکت ها</li>
@endpush
    ...
    <div class="card card-flush">
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                <a href="{{ route('admin.tickets.priorities.create') }}" class="btn btn-primary">افزودن اولویت</a>
            </div>
        </div>
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_sales_table">
                        <thead>
                            <tr class="text-center text-gray-400 fw-bolder fs-6 text-uppercase gs-0">
                                ...
                                <th class="min-w-300px">نام اولویت</th>
                        ...
                        <tbody class="fw-bold text-gray-600 text-center">
                            @foreach ($ticketPriorities as $key => $ticketPriority)
                                <tr>
                                    ...
                                    <td>
                                        <span class="fw-bold text-hover-primary text-gray-800">{{ $ticketPriority->name }}</span>
                                    </td>
                                    <td>
                                        <div class="form-check form-check-solid form-switch ms-md-20">
                                            <input type="checkbox" name="status" id="{{ $ticketPriority->id }}" class="form-check-input w-35px h-20px" onchange="changeStatus({{ $ticketPriority->id }})" data-url="{{ route('admin.tickets.priorities.change-status', [$ticketPriority->id]) }}" @checked($ticketPriority->status === 1)>
                                        </div>
                                    </td>
                                    <td>
                                        <a href="{{ route('admin.tickets.priorities.edit', [$ticketPriority->id]) }}" class="btn btn-icon btn-bg-light btn-active-light-primary w-30px h-30px me-3">
                                        ...
                                        <form action="{{ route('admin.tickets.priorities.destroy', [$ticketPriority->id]) }}" method="post" class="d-inline">
                                            @csrf
                                            @method('delete')
                ...
                success: function(response) {
                    if (response.status) {
                        if (response.checked) {
                            ...
                            successToastr('اولویت با موفقیت فعال شد');
                        } else {
                            element.prop('checked', false);
                            successToastr('اولویت با موفقیت غیر فعال شد');
                        }
                    } else {
