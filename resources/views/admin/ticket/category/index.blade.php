...
@section('title', 'لیست دسته بندی تیکت ها')

@section('page-title', 'لیست دسته بندی تیکت ها')

@push('breadcrumb')
    <li class="breadcrumb-item">
        <span class="bullet bg-gray-300 w-5px h-2px"></span>
    </li>
    <li class="breadcrumb-item text-dark">دسته بندی تیکت ها</li>
@endpush
    ...
    <div class="card card-flush">
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                <a href="{{ route('admin.tickets.categories.create') }}" class="btn btn-primary">افزودن دسته بندی</a>
            </div>
        </div>
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_sales_table">
                        <thead>
                            <tr class="text-center text-gray-400 fw-bolder fs-6 text-uppercase gs-0">
                                ...
                                <th class="min-w-300px">نام دسته بندی</th>
                        ...
                        <tbody class="fw-bold text-gray-600 text-center">
                            @foreach ($ticketCategories as $key => $ticketCategory)
                                <tr>
                                    ...
                                    <td>
                                        <span class="fw-bold text-hover-primary text-gray-800">{{ $ticketCategory->name }}</span>
                                    </td>
                                    <td>
                                        <div class="form-check form-check-solid form-switch ms-md-20">
                                            <input type="checkbox" name="status" id="{{ $ticketCategory->id }}" class="form-check-input w-35px h-20px" onchange="changeStatus({{ $ticketCategory->id }})" data-url="{{ route('admin.tickets.categories.change-status', [$ticketCategory->id]) }}" @checked($ticketCategory->status === 1)>
                                        </div>
                                    </td>
                                    <td>
                                        <a href="{{ route('admin.tickets.categories.edit', [$ticketCategory->id]) }}" class="btn btn-icon btn-bg-light btn-active-light-primary w-30px h-30px me-3">
                                        ...
                                        <form action="{{ route('admin.tickets.categories.destroy', [$ticketCategory->id]) }}" method="post" class="d-inline">
                                            @csrf
                                            @method('delete')
