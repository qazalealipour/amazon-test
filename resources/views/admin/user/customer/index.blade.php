...
@push('breadcrumb')
    ...
    <li class="breadcrumb-item text-dark">مشتریان</li>
@endpush
    ...
    <div class="card card-flush">
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                <a href="{{ route('admin.users.customers.create') }}" class="btn btn-primary">افزودن مشتری</a>
            </div>
        </div>
                                ...
                                <tr>
                                    ...
                                    <td class="d-flex align-items-center text-start">
                                        <div class="symbol symbol-circle symbol-50px overflow-hidden me-3">
                                            @if ($customer->profile_photo_path === null)
                                                <span>
                                                    <div class="symbol-label fs-3 bg-light-primary text-primary">{{ Illuminate\Support\Str::substr($customer->first_name, 0, 1) }}</div>
                                                </span>
                                            @else
                                                <span>
                                                    <div class="symbol-label">
                                                        <img src="{{ asset($customer->profile_photo_path) }}" class="w-100">
                                                    </div>
                                                </span>
                                            @endif
                                        </div>
                                        <div class="d-flex flex-column">
                                            <span class="text-gray-800 text-hover-primary mb-1">{{ $customer->full_name }}</span>
                                    ...
                                    <td>
                                        <div class="form-check form-check-solid form-switch ms-11">
                                            <input type="checkbox" name="activation" id="activation-{{ $customer->id }}" class="form-check-input w-35px h-20px" onchange="changeActivation({{ $customer->id }})" data-url="{{ route('admin.users.customers.change-activation', [$customer->id]) }}" @checked($customer->activation === 1)>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="form-check form-check-solid form-switch ms-7">
                                            <input type="checkbox" name="status" id="status-{{ $customer->id }}" class="form-check-input w-35px h-20px" onchange="changeStatus({{ $customer->id }})" data-url="{{ route('admin.users.customers.change-status', [$customer->id]) }}" @checked($customer->status === 1)>
                                        </div>
                                    </td>
                                    <td>
                                        <a href="{{ route('admin.users.customers.edit', [$customer->id]) }}" class="btn btn-icon btn-bg-light btn-active-light-primary w-30px h-30px me-3">
                                        ...
                                        <form action="{{ route('admin.users.customers.destroy', [$customer->id]) }}" method="post" class="d-inline">
                                            @csrf
                                            @method('delete')
