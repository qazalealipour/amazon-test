...
@push('breadcrumb')
    ...
    <li class="breadcrumb-item text-dark">ویرایش مشتری</li>
@endpush
...
@section('content')
    <div class="card">
        ...
        <form id="form" class="form" action="{{ route('admin.users.customers.update', [$customer->id]) }}" enctype="multipart/form-data" method="post">
            @csrf
            @method('put')
            <div class="card-body p-9">
                <div class="row mb-5">
                    ...
                    <div class="col-lg-8">
                        ...
                        <div class="image-input image-input-circle" data-kt-image-input="true" style="background-image: url({{ asset('admin-assets/media/svg/avatars/blank.svg') }})">
                            <div class="image-input-wrapper w-150px h-150px bgi-position-center" style="background-size: 100%; background-image: url({{ asset(Illuminate\Support\Str::replace('\\', '/', $customer->profile_photo_path)) }})"></div>
                ...
                <div class="row mb-8">
                    ...
                    <div class="col-xl-9 fv-row">
                        <input type="text" class="form-control form-control-solid" name="first_name" value="{{ old('first_name', $customer->first_name) }}" />
                        @error('first_name')
                            <div class="fv-plugins-message-container invalid-feedback">
                                <div class="badge badge-light-danger">{{ $message }}</div>
                            </div>
                ...
                <div class="row mb-8">
                    ...
                    <div class="col-xl-9 fv-row">
                        <input type="text" class="form-control form-control-solid" name="last_name" value="{{ old('last_name', $customer->last_name) }}" />
                        @error('last_name')
                            <div class="fv-plugins-message-container invalid-feedback">
                                <div class="badge badge-light-danger">{{ $message }}</div>
                            </div>
                    ...
                    <div class="col-xl-9 fv-row">
                        <div class="w-100">
                            <select class="form-select form-select-solid" name="activation" data-control="select2"
                                ...
                                <option value="0" @selected(old('activation', $customer->activation) == 0)>غیر فعال</option>
                                <option value="1" @selected(old('activation', $customer->activation) == 1)>فعال</option>
                            </select>
                        </div>
                        @error('activation')
                            <div class="fv-plugins-message-container invalid-feedback">
