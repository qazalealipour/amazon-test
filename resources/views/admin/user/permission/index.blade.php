...
@section('page-title', 'لیست دسترسی ها')
    ...
    <div class="card card-flush">
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                <a href="{{ route('admin.users.permissions.create') }}" class="btn btn-primary">افزودن دسترسی</a>
            </div>
        </div>
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_sales_table">
                        <thead>
                            <tr class="text-center text-gray-400 fw-bolder fs-6 text-uppercase gs-0">
                                ...
                                <th class="min-w-250px">عنوان دسترسی</th>
                                <th class="min-w-300px">نقش ها</th>
                        ...
                        <tbody class="fw-bold text-gray-600 text-center">
                            @foreach ($permissions as $permission)
                                <tr>
                                    ...
                                    <td data-kt-ecommerce-order-filter="order_id">
                                        <span class="text-gray-800 text-hover-primary fw-bold">{{ $permission->name }}</span>
                                    </td>
                                    <td>
                                        @if (empty($permission->roles()->get()->toArray()))
                                            <div class="badge badge-light-danger fw-bolder">برای این نقش هیچ سطح دسترسی تعریف نشده است</div>
                                        @else
                                    ...
                                    <td>
                                        <div class="form-check form-check-solid form-switch ms-7">
                                            <input type="checkbox" name="status" id="{{ $permission->id }}" class="form-check-input w-35px h-20px" onchange="changeStatus({{ $permission->id }})" data-url="{{ route('admin.users.permissions.change-status', [$permission->id]) }}" @checked($permission->status === 1)>
                                        </div>
                                    </td>
                                    <td>
                                        <a href="{{ route('admin.users.permissions.edit', [$permission->id]) }}" class="btn btn-icon btn-bg-light btn-active-light-primary w-30px h-30px me-3">
                                        ...
                                        <form action="{{ route('admin.users.permissions.destroy', [$permission->id]) }}" method="post" class="d-inline">
                                            @csrf
                                            @method('delete')
                ...
                success: function(response) {
                    if (response.status) {
                        if (response.checked) {
                            ...
                            successToastr('دسترسی با موفقیت فعال شد');
                        } else {
                            element.prop('checked', false);
                            successToastr('دسترسی با موفقیت غیر فعال شد');
                        }
                    } else {
