...
@section('content')
    <div class="card">
        ...
        <form id="form" class="form" action="{{ route('admin.users.permissions.update', [$permission->id]) }}" method="post">
            @csrf
            @method('put')
            <div class="card-body p-9">
                <div class="row mb-8">
                    ...
                    <div class="col-xl-9 fv-row">
                        <input type="text" class="form-control form-control-solid" name="name" value="{{ old('name', $permission->name) }}" />
                        @error('name')
                    ...
                    <div class="col-xl-9 fv-row">
                        <div class="w-100">
                            <select class="form-select form-select-solid" name="status" data-control="select2"
                                ...
                                <option value="0" @selected(old('status', $permission->status) == 0)>غیر فعال</option>
                                <option value="1" @selected(old('status', $permission->status) == 1)>فعال</option>
                            </select>
                        </div>
                        @error('status')
                            <div class="fv-plugins-message-container invalid-feedback">
                ...
                <div class="row mb-8">
                    ...
                    <div class="col-xl-9 fv-row">
                        <textarea name="description" class="form-control form-control-solid h-100px">{{ old('description', $permission->description) }}</textarea>
                        @error('description')
                            <div class="fv-plugins-message-container invalid-feedback">
                                <div class="badge badge-light-danger">{{ $message }}</div>
                            </div>
