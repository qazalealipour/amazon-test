...
@section('title', 'لیست مقادیر فرم کالا')

@section('page-title', 'لیست مقادیر فرم کالا')
...
@push('breadcrumb')
    ...
    <li class="breadcrumb-item text-dark">مقادیر فرم کالا</li>
@endpush
    ...
    <div class="card card-flush">
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                <a href="{{ route('admin.market.properties.values.create', [$property->id]) }}" class="btn btn-primary">افزودن مقدار فرم</a>
            </div>
        </div>
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_sales_table">
                        <thead>
                            <tr class="text-center text-gray-400 fw-bolder fs-6 text-uppercase gs-0">
                                ...
                                <th class="min-w-200px">مقدار فرم</th>
                                <th class="min-w-250px">نام محصول</th>
                        ...
                        <tbody class="fw-bold text-gray-600 text-center">
                            @foreach ($property->values as $value)
                                <tr>
                                    ...
                                    <td>
                                        <span class="fw-bold text-hover-primary text-gray-800">{{ json_decode($value->value)->value }}</span>
                                    </td>
                                    <td>{{ $value->product->name }}</td>
                                    <td>
                                        <span class="fw-bold text-hover-primary text-gray-800">{{ number_format(json_decode($value->value)->price_increase) }}</span>
                                    </td>
                                    <td>{{ $value->type == 0 ? 'ساده' : 'انتخابی' }}</td>
                                    <td>
                                        <a href="{{ route('admin.market.properties.values.edit', ['property' => $property->id, 'value' => $value->id]) }}" class="btn btn-icon btn-bg-light btn-active-light-primary w-30px h-30px me-3">
                                        ...
                                        <form action="{{ route('admin.market.properties.values.destroy', ['property' => $property->id, 'value' => $value->id]) }}" method="post" class="d-inline">
                                            @csrf
                                            @method('delete')
