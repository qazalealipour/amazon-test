...
@section('title', 'لیست گالری')

@section('page-title', 'لیست گالری')
...
@push('breadcrumb')
    ...
    <li class="breadcrumb-item text-dark">گالری</li>
@endpush
    ...
    <div class="card card-flush">
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                <a href="{{ route('admin.market.products.gallery.create', [$product->id]) }}" class="btn btn-primary">افزودن عکس به گالری</a>
            </div>
        </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="symbol symbol-50px">
                                            <img src="{{ asset($image->image_path['indexArray']['small']) }}" class="symbol-label">
                                        </div>
                                    </td>
                                    <td>{{ $product->name }}</td>
                                    <td>
                                        <form action="{{ route('admin.market.products.gallery.destroy', ['product' => $product->id, 'image' => $image->id]) }}" method="post" class="d-inline">
                                            @csrf
                                            @method('delete')
