...
@push('breadcrumb')
    ...
    <li class="breadcrumb-item text-dark">برندها</li>
@endpush
    ...
    <div class="card card-flush">
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                <a href="{{ route('admin.market.brands.create') }}" class="btn btn-primary">افزودن برند</a>
            </div>
        </div>
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_sales_table">
                        <thead>
                            <tr class="text-center text-gray-400 fw-bolder fs-6 text-uppercase gs-0">
                                ...
                                <th class="min-w-150px">نام فارسی برند</th>
                                <th class="min-w-150px">نام اصلی برند</th>
                                    ...
                                    <td>
                                        <div class="symbol symbol-50px">
                                            @if ($brand->logo_path === null)
                                                <span class="symbol-label bg-light-warning text-warning fw-bold">{{ Illuminate\Support\Str::substr($brand->original_name, 0, 1) }}</span>
                                            @else
                                                <img src="{{ asset($brand->logo_path) }}" class="symbol-label">
                                            @endif
                                        </div>
                                    </td>
                                    <td>
                                        <span class="fw-bold text-hover-primary text-gray-800">{{ $brand->persian_name }}</span>
                                    </td>
                                    <td>
                                        <span class="fw-bold text-hover-primary text-gray-800">{{ $brand->original_name }}</span>
                                    </td>
                                    <td>
                                        @foreach (explode(',', $brand->tags) as $brandTag)
                                            <span class="badge badge-light fw-bolder">{{ $brandTag }}</span>
                                        @endforeach
                                    </td>
                                    <td>
                                        <div class="form-check form-check-solid form-switch ms-7">
                                            <input type="checkbox" name="status" id="{{ $brand->id }}" class="form-check-input w-35px h-20px" onchange="changeStatus({{ $brand->id }})" data-url="{{ route('admin.market.brands.change-status', [$brand->id]) }}" @checked($brand->status === 1)>
                                        </div>
                                    </td>
                                    <td>
                                        <a href="{{ route('admin.market.brands.edit', [$brand->id]) }}" class="btn btn-icon btn-bg-light btn-active-light-primary w-30px h-30px me-3">
                                        ...
                                        <form action="{{ route('admin.market.brands.destroy', [$brand->id]) }}" method="post" class="d-inline">
                                            @csrf
                                            @method('delete')
                ...
                success: function(response) {
                    if (response.status) {
                        if (response.checked) {
                            ...
                            successToastr('برند با موفقیت فعال شد');
                        } else {
                            element.prop('checked', false);
                            successToastr('برند با موفقیت غیر فعال شد');
                        }
                    } else {
