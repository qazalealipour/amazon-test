...
@section('title', 'لیست رنگ های کالا')

@section('page-title', 'لیست رنگ های کالا')
...
@push('breadcrumb')
    ...
    <li class="breadcrumb-item text-dark">رنگ های کالا</li>
@endpush
    ...
    <div class="card card-flush">
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                <a href="{{ route('admin.market.products.colors.create', [$product->id]) }}" class="btn btn-primary">افزودن رنگ کالا</a>
            </div>
        </div>
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_sales_table">
                        <thead>
                            <tr class="text-center text-gray-400 fw-bolder fs-6 text-uppercase gs-0">
                                ...
                                <th class="min-w-150px">نام رنگ</th>
                        ...
                        <tbody class="fw-bold text-gray-600 text-center">
                            @foreach ($product->colors as $color)
                                <tr>
                                    ...
                                    <td>
                                        <span class="fw-bold text-hover-primary text-gray-800">{{ $color->color_name }}</span>
                                    </td>
                                    <td>{{ number_format($color->price_increase) }} تومان</td>
                                    <td>{{ $product->name }}</td>
                                    <td>
                                        <form action="{{ route('admin.market.products.colors.destroy', ['product' => $product->id, 'color' => $color->id]) }}" method="post" class="d-inline">
                                            @csrf
                                            @method('delete')
