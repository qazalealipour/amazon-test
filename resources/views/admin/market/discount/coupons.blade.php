...
@section('title', 'لیست کوپن های تخفیف')

@section('page-title', 'لیست کوپن های تخفیف')
...
@push('breadcrumb')
    ...
    <li class="breadcrumb-item text-dark">کوپن های تخفیف</li>
@endpush
    ...
    <div class="card card-flush">
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                <a href="{{ route('admin.market.discounts.coupons.create') }}" class="btn btn-primary">افزودن تخفیف عمومی</a>
            </div>
        </div>
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_sales_table">
                        <thead>
                            <tr class="text-center text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                                ...
                                <th class="min-w-150px">کد تخفیف</th>
                                <th class="min-w-200px">میزان تخفیف</th>
                                <th class="min-w-200px">سقف تخفیف</th>
                                <th class="min-w-150px">نوع کوپن</th>
                                <th class="min-w-250px">تاریخ شروع</th>
                                <th class="min-w-250px">تاریخ پایان</th>
                        ...
                        <tbody class="fw-bold text-gray-600 text-center">
                            @foreach ($coupons as $coupon)
                                <tr>
                                    ...
                                    <td>
                                        <span class="text-gray-800 text-hover-primary fw-bold">{{ $coupon->code }}</span>
                                    </td>
                                    @if ($coupon->amount_type == 0)
                                        <td>{{ $coupon->amount }}%</td>
                                    @else
                                        <td>{{ number_format($coupon->amount) }} تومان</td> 
                                    @endif
                                    <td>{{ number_format($coupon->discount_ceiling) . 'تومان' ?? '-' }}</td>
                                    <td>
                                        <span class="badge badge-light-info fw-bolder px-4 py-3">
                                            {{ $coupon->type == 0 ? 'عمومی' : 'خصوصی' }}
                                        </span>
                                    </td>
                                    <td>{{ jalaliDate($coupon->start_date) }}</td>
                                    <td>{{ jalaliDate($coupon->end_date) }}</td>
                                    <td>
                                        <a href="{{ route('admin.market.discounts.coupons.edit', [$coupon->id]) }}" class="btn btn-icon btn-bg-light btn-active-light-primary w-30px h-30px me-3">
                                        ...
                                        <form action="{{ route('admin.market.discounts.coupons.destroy', [$coupon->id]) }}" method="post" class="d-inline">
                                            @csrf
                                            @method('delete')
