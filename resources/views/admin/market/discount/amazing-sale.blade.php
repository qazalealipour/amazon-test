...
@section('page-title', 'فروش شگفت انگیز')
...
@push('breadcrumb')
    ...
    <li class="breadcrumb-item text-dark">فروش شگفت انگیز</li>
@endpush
    ...
    <div class="card card-flush">
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                <a href="{{ route('admin.market.discounts.amazing-sale.create') }}" class="btn btn-primary">افزودن تخفیف عمومی</a>
            </div>
        </div>
                        ...
                        <tbody class="fw-bold text-gray-600 text-center">
                            @foreach ($amazingSales as $amazingSale)
                                <tr>
                                    ...
                                    <td>
                                        <span class="text-gray-800 text-hover-primary fw-bold">{{ $amazingSale->product->name }}</span>
                                    </td>
                                    <td>{{ $amazingSale->percentage }}%</td>
                                    <td>{{ jalaliDate($amazingSale->start_date) }}</td>
                                    <td>{{ jalaliDate($amazingSale->end_date) }}</td>
                                    <td>
                                        <a href="{{ route('admin.market.discounts.amazing-sale.edit', [$amazingSale->id]) }}" class="btn btn-icon btn-bg-light btn-active-light-primary w-30px h-30px me-3">
                                        ...
                                        <form action="{{ route('admin.market.discounts.amazing-sale.destroy', [$amazingSale->id]) }}" method="post" class="d-inline">
                                            @csrf
                                            @method('delete')
