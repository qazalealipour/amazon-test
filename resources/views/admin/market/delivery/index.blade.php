...
@section('title', 'لیست روش های ارسال')

@section('page-title', 'لیست روش های ارسال')

@push('breadcrumb')
    <li class="breadcrumb-item">
        <span class="bullet bg-gray-300 w-5px h-2px"></span>
    </li>
    <li class="breadcrumb-item text-dark">روش های ارسال</li>
@endpush
    ...
    <div class="card card-flush">
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                <a href="{{ route('admin.market.delivery.create') }}" class="btn btn-primary">افزودن روش ارسال</a>
            </div>
        </div>
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_sales_table">
                        <thead>
                            <tr class="text-center text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                                ...
                                <th class="min-w-170px">عنوان روش ارسال</th>
                                <th class="min-w-170px">هزینه ارسال</th>
                                <th class="min-w-170px">زمان ارسال</th>
                        ...
                        <tbody class="fw-bold text-gray-600 text-center">
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="text-gray-800 text-hover-primary fs-5 fw-bold">{{ $delivery->name }}</span>
                                </td>
                                ...
                                <td>
                                    <div class="form-check form-check-solid form-switch ms-7">
                                        <input type="checkbox" name="status" id="{{ $delivery->id }}" class="form-check-input w-35px h-20px" onchange="changeStatus({{ $delivery->id }})" data-url="{{ route('admin.market.delivery.change-status', [$delivery->id]) }}" @checked($delivery->status === 1)>
                                    </div>
                                </td>
                                <td>
                                    <a href="{{ route('admin.market.delivery.edit', [$delivery->id]) }}" class="btn btn-icon btn-bg-light btn-active-light-primary w-30px h-30px me-3">
                ...
                success: function(response) {
                    if (response.status) {
                        if (response.checked) {
                            ...
                            successToastr('روش ارسال با موفقیت فعال شد');
                        } else {
                            element.prop('checked', false);
                            successToastr('روش ارسال با موفقیت غیر فعال شد');
                        }
                    } else {
