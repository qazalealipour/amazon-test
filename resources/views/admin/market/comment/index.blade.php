                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="form-check form-check-solid form-switch ms-7">
                                        <input type="checkbox" name="status" id="status-{{ $comment->id }}" class="form-check-input w-35px h-20px" onchange="changeStatus({{ $comment->id }})" data-url="{{ route('admin.market.comments.change-status', [$comment->id]) }}" @checked($comment->status === 1)>
                                    </div>
                                </td>
                                <td>
                                    <a href="{{ route('admin.market.comments.show', [$comment->id]) }}" class="btn btn-icon btn-bg-light btn-active-light-primary w-30px h-30px me-3">
                                        <span data-bs-toggle="tooltip" data-bs-trigger="hover" title="" data-bs-original-title="نمایش">
                                            <i class="bi bi-eye-fill fs-4"></i>
                                        </span>
                                    </a>
                                    @if ($comment->approved == 1)
                                        <a href="{{ route('admin.market.comments.change-approved', [$comment->id]) }}" class="btn btn-icon btn-bg-light btn-active-light-primary w-30px h-30px me-3">
