...
@push('breadcrumb')
    ...
    <li class="breadcrumb-item text-dark">کالاها</li>
@endpush
    ...
    <div class="card card-flush">
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                <a href="{{ route('admin.market.products.create') }}" class="btn btn-primary">افزودن کالا</a>
            </div>
        </div>
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_sales_table">
                        <thead>
                            <tr class="text-center text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                                ...
                                <th class="min-w-200px">قیمت</th>
                                <th class="min-w-175px">دسته بندی</th>
                                <th class="min-w-200px">برچسب ها</th>
                                <th class="min-w-225px">تاریخ انتشار</th>
                                <th class="min-w-125px">قابل فروش</th>
                                <th class="min-w-100px">وضعیت</th>
                                <th class="min-w-225px">عملیات</th>
                            </tr>
                        </thead>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @foreach (explode(',', $product->tags) as $productTag)
                                            <span class="badge badge-light fw-bolder">{{ $productTag }}</span>
                                        @endforeach
                                    </td>
                                    <td>{{ jalaliDate($product->published_at) }}</td>
                                    <td>
                                        <div class="form-check form-check-solid form-switch ms-10">
                                            <input type="checkbox" name="marketable" id="marketable-{{ $product->id }}" class="form-check-input w-35px h-20px" onchange="changeMarketable({{ $product->id }})" data-url="{{ route('admin.market.products.change-marketable', [$product->id]) }}" @checked($product->marketable === 1)>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="form-check form-check-solid form-switch ms-7">
                                            <input type="checkbox" name="status" id="status-{{ $product->id }}" class="form-check-input w-35px h-20px" onchange="changeStatus({{ $product->id }})" data-url="{{ route('admin.market.products.change-status', [$product->id]) }}" @checked($product->status === 1)>
                                        </div>
                                    </td>
                                    <td>
                                        <a href="{{ route('admin.market.products.gallery.index', [$product->id]) }}" class="btn btn-icon btn-bg-light btn-active-light-primary w-30px h-30px me-3">
                                            <span data-bs-toggle="tooltip" data-bs-trigger="hover" title="" data-bs-original-title="گالری">
                                        ...
                                        <a href="{{ route('admin.market.products.colors.index', [$product->id]) }}" class="btn btn-icon btn-bg-light btn-active-light-primary w-30px h-30px me-3">
                                            <span data-bs-toggle="tooltip" data-bs-trigger="hover" title="" data-bs-original-title="رنگ های کالا">
                                                <span class="svg-icon svg-icon-3">
                                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <a href="{{ route('admin.market.products.guarantees.index', [$product->id]) }}" class="btn btn-icon btn-bg-light btn-active-light-primary w-30px h-30px me-3">
                                            <span data-bs-toggle="tooltip" data-bs-trigger="hover" title="" data-bs-original-title="گارانتی">
                                                <i class="fa fa-shield-alt"></i>
                                            </span>
                                        </a>
                                        <a href="{{ route('admin.market.products.edit', [$product->id]) }}" class="btn btn-icon btn-bg-light btn-active-light-primary w-30px h-30px me-3">
                                        ...
                                        <form action="{{ route('admin.market.products.destroy', [$product->id]) }}" method="post" class="d-inline">
                                            @csrf
                                            @method('delete')
                ...
                success: function(response) {
                    if (response.status) {
                        if (response.checked) {
                            ...
                            successToastr('کالا با موفقیت فعال شد');
                        } else {
                            element.prop('checked', false);
                            successToastr('کالا با موفقیت غیر فعال شد');
                        }
                    } else {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.marketable) {
                        if (response.checked) {
                            element.prop('checked', true);
                            successToastr('کالا قابل فروش می باشد');
                        } else {
                            element.prop('checked', false);
                            successToastr('کالا قابل فروش نمی باشد');
                        }
                    } else {
                        element.prop('checked', elementValue);
                        errorToastr('هنگام ویرایش مشکلی پیش آمده است');
