...
@section('page-title', 'لیست بنرها')

@push('breadcrumb')
    <li class="breadcrumb-item">
        <span class="bullet bg-gray-300 w-5px h-2px"></span>
    </li>
    <li class="breadcrumb-item text-dark">بنرها</li>
@endpush
    ...
    <div class="card card-flush">
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                <a href="{{ route('admin.content.banners.create') }}" class="btn btn-primary">افزودن بنر</a>
            </div>
        </div>
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_sales_table">
                        <thead>
                            <tr class="text-center text-gray-400 fw-bolder fs-6 text-uppercase gs-0">
                                ...
                                <th class="min-w-250px">عنوان بنر</th>
                                <th class="min-w-200px">آدرس</th>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="symbol symbol-50px">
                                            <img src="{{ asset($banner->image) }}" class="symbol-label">
                                        </div>
                                    </td>
                                    <td>
                                        <span class="fw-bold text-hover-primary text-gray-800">{{ $banner->title }}</span>
                                    </td>
                                    <td>{{ $banner->url }}</td>
                                    <td>{{ $positions[$banner->position] }}</td>
                                    <td>
                                        <div class="form-check form-check-solid form-switch ms-7">
                                            <input type="checkbox" name="status" id="{{ $banner->id }}" class="form-check-input w-35px h-20px" onchange="changeStatus({{ $banner->id }})" data-url="{{ route('admin.content.banners.change-status', [$banner->id]) }}" @checked($banner->status === 1)>
                                        </div>
                                    </td>
                                    <td>
                                        <a href="{{ route('admin.content.banners.edit', [$banner->id]) }}" class="btn btn-icon btn-bg-light btn-active-light-primary w-30px h-30px me-3">
                                        ...
                                        <form action="{{ route('admin.content.banners.destroy', [$banner->id]) }}" method="post" class="d-inline">
                                            @csrf
                                            @method('delete')
                ...
                success: function(response) {
                    if (response.status) {
                        if (response.checked) {
                            ...
                            successToastr('بنر با موفقیت فعال شد');
                        } else {
                            element.prop('checked', false);
                            successToastr('بنر با موفقیت غیر فعال شد');
                        }
                    } else {
