    ...
    <div class="card card-flush">
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                <a href="{{ route('admin.content.categories.create') }}" class="btn btn-primary">افزودن دسته بندی</a>
            </div>
        </div>
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_sales_table">
                        <thead>
                            <tr class="text-center text-gray-400 fw-bolder fs-6 text-uppercase gs-0">
                                ...
                                <th class="min-w-150px">نام دسته بندی</th>
                                    ...
                                    <td>
                                        <div class="symbol symbol-50px">
                                            @if ($postCategory->image_path === null)
                                                <span class="symbol-label bg-light-warning text-warning fw-bold">{{ Illuminate\Support\Str::substr($postCategory->name, 0, 1) }}</span>
                                            @else
                                                <img src="{{ asset($postCategory->image_path['indexArray']['small']) }}" class="symbol-label">
                                            @endif
                                        </div>
                                    </td>
                                    <td>
                                        <span class="fw-bold text-hover-primary text-gray-800">{{ $postCategory->name }}</span>
                                    </td>
                                    <td>
                                        <span class="fw-bold text-hover-primary text-gray-800">{{ $postCategory->description }}</span>
                                    </td>
                                    <td>
                                        @foreach (explode(',', $postCategory->tags) as $postCategoryTag)
                                            <span class="badge badge-light fw-bolder">{{ $postCategoryTag }}</span>
                                        @endforeach
                                    </td>
                                    <td>
                                        <div class="form-check form-check-solid form-switch ms-7">
                                            <input type="checkbox" name="status" id="{{ $postCategory->id }}" class="form-check-input w-35px h-20px" onchange="changeStatus({{ $postCategory->id }})" data-url="{{ route('admin.content.categories.change-status', [$postCategory->id]) }}" @checked($postCategory->status === 1)>
                                        </div>
                                    </td>
                                    <td>
                                        <a href="{{ route('admin.content.categories.edit', [$postCategory->id]) }}" class="btn btn-icon btn-bg-light btn-active-light-primary w-30px h-30px me-3">
                                        ...
                                        <form action="{{ route('admin.content.categories.destroy', [$postCategory->id]) }}" method="post" class="d-inline">
                                            @csrf
                                            @method('delete')
