...
@section('title', 'لیست سوالات متداول')

@section('page-title', 'لیست سوالات متداول')
    ...
    <div class="card card-flush">
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                <a href="{{ route('admin.content.faqs.create') }}" class="btn btn-primary">افزودن سوال</a>
            </div>
        </div>
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_sales_table">
                        <thead>
                            <tr class="text-center text-gray-400 fw-bolder fs-6 text-uppercase gs-0">
                                ...
                                <th class="min-w-300px">پرسش</th>
                                <th class="min-w-400px">پاسخ</th>
                        ...
                        <tbody class="fw-bold text-gray-600 text-center">
                            @foreach ($faqs as $key => $faq)
                                <tr>
                                    ...
                                    <td class="pe-0">
                                        <span class="fw-bold text-hover-primary text-gray-800">{{ $faq->question }}</span>
                                    </td>
                                    <td class="pe-0">
                                        <span class="fw-bold text-hover-primary text-gray-800">{{ $faq->answer }}</span>
                                    </td>
                                    <td class="pe-0">
                                        @foreach (explode(',', $faq->tags) as $faqTag)
                                            <span class="badge badge-light fw-bolder">{{ $faqTag }}</span>
                                        @endforeach
                                    </td>
                                    <td class="pe-0">
                                        <div class="form-check form-check-solid form-switch ms-7">
                                            <input type="checkbox" name="status" id="{{ $faq->id }}" class="form-check-input w-35px h-20px" onchange="changeStatus({{ $faq->id }})" data-url="{{ route('admin.content.faqs.change-status', [$faq->id]) }}" @checked($faq->status === 1)>
                                        </div>
                                    </td>
                                    <td>
                                        <a href="{{ route('admin.content.faqs.edit', [$faq->id]) }}" class="btn btn-icon btn-bg-light btn-active-light-primary w-30px h-30px me-3">
                                        ...
                                        <form action="{{ route('admin.content.faqs.destroy', [$faq->id]) }}" method="post" class="d-inline">
                                            @csrf
                                            @method('delete')
