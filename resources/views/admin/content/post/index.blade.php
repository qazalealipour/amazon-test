...
@push('breadcrumb')
    ...
    <li class="breadcrumb-item text-dark">پست ها</li>
@endpush
    ...
    <div class="card card-flush">
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                <a href="{{ route('admin.content.posts.create') }}" class="btn btn-primary">افزودن پست</a>
            </div>
        </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="symbol symbol-50px">
                                            <img src="{{ asset($post->image_path['indexArray']['small']) }}" class="symbol-label">
                                        </div>
                                    </td>
                                    <td>
                                        <span class="fw-bold text-hover-primary text-gray-800">{{ $post->title }}</span>
                                    </td>
                                    <td>{{ $post->postCategory->name }}</td>
                                    <td>{{ $post->author->full_name }}</td>
                                    <td>
                                        <span class="fw-bold text-hover-primary text-gray-800">{{ Illuminate\Support\Str::limit($post->summary, 90) }}</span>
                                    </td>
                                    <td>
                                        @foreach (explode(',', $post->tags) as $postTag)
                                            <span class="badge badge-light fw-bolder">{{ $postTag }}</span>
                                        @endforeach
                                    </td>
                                    <td>{{ jalaliDate($post->published_at) }}</td>
                                    <td>
                                        <div class="form-check form-check-solid form-switch ms-20">
                                            <input type="checkbox" name="commentable" id="commentable-{{ $post->id }}" class="form-check-input w-35px h-20px" onchange="changeCommentable({{ $post->id }})" data-url="{{ route('admin.content.posts.change-commentable', [$post->id]) }}" @checked($post->commentable === 1)>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="form-check form-check-solid form-switch ms-7">
                                            <input type="checkbox" name="status" id="status-{{ $post->id }}" class="form-check-input w-35px h-20px" onchange="changeStatus({{ $post->id }})" data-url="{{ route('admin.content.posts.change-status', [$post->id]) }}" @checked($post->status === 1)>
                                        </div>
                                    </td>
                                    <td>
                                        <a href="{{ route('admin.content.posts.edit', [$post->id]) }}" class="btn btn-icon btn-bg-light btn-active-light-primary w-30px h-30px me-3">
                                        ...
                                        <form action="{{ route('admin.content.posts.destroy', [$post->id]) }}" method="post" class="d-inline">
                                            @csrf
                                            @method('delete')
                ...
                success: function(response) {
                    if (response.status) {
                        if (response.checked) {
                            ...
                            successToastr('پست با موفقیت فعال شد');
                        } else {
                            element.prop('checked', false);
                            successToastr('پست با موفقیت غیر فعال شد');
                        }
                    } else {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.commentable) {
                        if (response.checked) {
                            element.prop('checked', true);
                            successToastr('امکان درج کامنت برای پست فعال شد');
                        } else {
                            element.prop('checked', false);
                            successToastr('امکان درج کامنت برای پست غیر فعال شد');
                        }
                    } else {
                        element.prop('checked', elementValue);
                        errorToastr('هنگام ویرایش مشکلی پیش آمده است');
