...
@push('breadcrumb')
    ...
    <li class="breadcrumb-item text-muted">پست ها</li>
    <li class="breadcrumb-item">
        <span class="bullet bg-gray-300 w-5px h-2px"></span>
    </li>
    <li class="breadcrumb-item text-dark">ویرایش پست</li>
@endpush

@section('content')
    <div class="card">
        <div class="card-header">
            <div class="card-title fs-3 fw-bolder">مشخصات پست</div>
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                <a href="{{ route('admin.content.posts.index') }}" class="btn btn-primary">بازگشت</a>
            </div>
        </div>
        <form id="form" class="form fv-plugins-bootstrap5 fv-plugins-framework" action="{{ route('admin.content.posts.update', [$post->id]) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('put')
            <div class="card-body p-9">
                <div class="row mb-5">
                    <div class="col-xl-3 text-md-end">
                        <div class="fs-6 fw-bold mt-2 mb-3">تصویر پست</div>
                    </div>
                    <div class="col-lg-8">
                        <div class="image-input image-input-outline" data-kt-image-input="true" style="background-image: url({{ asset('admin-assets/media/svg/files/blank-image.svg') }})">
                            <div class="image-input-wrapper w-150px h-150px bgi-position-center" style="background-size: 100%; background-image: url({{ asset(Illuminate\Support\Str::replace('\\', '/', $post->image_path['indexArray']['medium'])) }})"></div>
                ...
                <div class="row mb-8">
                    <div class="col-xl-3 text-md-end">
                        <div class="fs-6 fw-bold mt-2 mb-3">عنوان پست</div>
                    </div>
                    <div class="col-xl-9 fv-row">
                        <input type="text" class="form-control form-control-solid" name="title" value="{{ old('title', $post->title) }}" />
                        @error('title')
                            <div class="fv-plugins-message-container invalid-feedback">
                                <div class="badge badge-light-danger">{{ $message }}</div>
                            </div>
                ...
                <div class="row mb-8">
                    <div class="col-xl-3 text-md-end">
                        <div class="fs-6 fw-bold mt-2 mb-3">خلاصه</div>
                    </div>
                    <div class="col-xl-9 fv-row">
                        <textarea name="summary" class="form-control form-control-solid h-100px">{{ old('summary', $post->summary) }}</textarea>
                        @error('summary')
                            <div class="fv-plugins-message-container invalid-feedback">
                                <div class="badge badge-light-danger">{{ $message }}</div>
                            </div>
                ...
                <div class="row mb-8">
                    ...
                    <div class="col-xl-9 fv-row">
                        <input type="hidden" id="tags" class="form-control form-control-solid" name="tags" value="{{ old('tags', $post->tags) }}" />
                        ...
                        <div class="w-100">
                            <select class="form-select form-select-solid" name="category_id" data-control="select2"
                                ...
                                @foreach ($postCategories as $postCategory)
                                    <option value="{{ $postCategory->id }}" @selected(old('category_id', $post->category_id) == $postCategory->id)>{{ $postCategory->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        @error('category_id')
                ...
                <div class="row mb-8">
                    <div class="col-xl-3 text-md-end">
                        <div class="fs-6 fw-bold mt-2 mb-3">امکان درج کامنت</div>
                    </div>
                    <div class="col-xl-9 fv-row">
                        <div class="w-100">
                            <select class="form-select form-select-solid" name="commentable" data-control="select2"
                                data-hide-search="true">
                ...
                <div class="row mb-8">
                    ...
                    <div class="col-xl-9 fv-row fv-plugins-icon-container">
                        <div class="position-relative d-flex align-items-center">
                            ...
                            <input class="form-control form-control-solid ps-12 d-none" name="published_at" id="published_at" type="text" value="{{ old('published_at', $post->published_at) }}">
                            <input class="form-control form-control-solid ps-12" placeholder="انتخاب تاریخ" id="published_at_view" type="text" value="{{ old('published_at', $post->published_at) }}">
                        </div>
                ...
                <div class="row mb-8">
                    <div class="col-xl-3 text-md-end">
                        <div class="fs-6 fw-bold mt-2 mb-3">متن اصلی</div>
                    </div>
                    <div class="col-xl-9 fv-row">
                        <textarea name="body" id="editor" class="form-control form-control-solid h-100px">{{ old('body', $post->body) }}</textarea>
                        @error('body')
                            <div class="fv-plugins-message-container invalid-feedback">
                                <div class="badge badge-light-danger">{{ $message }}</div>
                            </div>
