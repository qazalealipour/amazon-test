...
@section('title', 'لیست پیج ها')

@section('page-title', 'لیست پیج ها')

@push('breadcrumb')
    <li class="breadcrumb-item">
        <span class="bullet bg-gray-300 w-5px h-2px"></span>
    </li>
    <li class="breadcrumb-item text-dark">پیج ساز</li>
@endpush
    ...
    <div class="card card-flush">
        <div class="card-header align-items-center py-5 gap-2 gap-md-5">
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                <a href="{{ route('admin.content.pages.create') }}" class="btn btn-primary">افزودن پیج</a>
            </div>
        </div>
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_sales_table">
                        <thead>
                            <tr class="text-center text-gray-400 fw-bolder fs-6 text-uppercase gs-0">
                                ...
                                <th class="min-w-300px">عنوان</th>
                                <th class="min-w-400px">محتوای پیج</th>
                        ...
                        <tbody class="fw-bold text-gray-600 text-center">
                            @foreach ($pages as $key => $page)
                                <tr>
                                    ...
                                    <td class="pe-0">
                                        <span class="fw-bold text-hover-primary text-gray-800">{{ $page->title }}</span>
                                    </td>
                                    <td class="pe-0">
                                        <span class="fw-bold text-hover-primary text-gray-800">{{ Illuminate\Support\Str::limit($page->body, 120) }}</span>
                                    </td>
                                    <td class="pe-0">
                                        @foreach (explode(',', $page->tags) as $pageTag)
                                            <span class="badge badge-light fw-bolder">{{ $pageTag }}</span>
                                        @endforeach
                                    </td>
                                    <td class="pe-0">
                                        <div class="form-check form-check-solid form-switch ms-7">
                                            <input type="checkbox" name="status" id="{{ $page->id }}" class="form-check-input w-35px h-20px" onchange="changeStatus({{ $page->id }})" data-url="{{ route('admin.content.faqs.change-status', [$page->id]) }}" @checked($page->status === 1)>
                                        </div>
                                    </td>
                                    <td>
                                        <a href="{{ route('admin.content.pages.edit', [$page->id]) }}" class="btn btn-icon btn-bg-light btn-active-light-primary w-30px h-30px me-3">
