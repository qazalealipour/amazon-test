                                    ...
                                    @foreach ($cartItems as $cartItem)
                                        ...
                                        <section
                                            ...
                                            <img class="flex-shrink-1"
                                                src="{{ asset($cartItem->product->image_path['indexArray']['medium']) }}"
                                                alt="">
            ...
            <nav class="">
                <section class="d-none d-md-flex justify-content-md-start position-relative">
                            ...
                            <section class="position-relative sublist-area">
                                ...
                                @foreach ($productCategories as $productCategory)
                                    <section class="sublist-item">
                                        <section class="sublist-item-toggle">{{ $productCategory->name }}</section>
                                        <section class="sublist-item-sublist">
                                            <section
                                                class="sublist-item-sublist-wrapper d-flex justify-content-around align-items-center">
                                                @foreach ($productCategory->children as $key => $subCategory)
                                                    <section class="sublist-column col">
                                                        <a href="{{ route('customer.category.products', [$subCategory->id]) }}"
                                                            class="sub-category">{{ $subCategory->name }}</a>
                                                        @foreach ($subCategory->children as $subSubCategory)
                                                            <a href="{{ route('customer.category.products', [$subSubCategory->id]) }}"
                    ...
                    @foreach ($menus as $menu)
                        <section class="navbar-item"><a href="{{ urldecode($menu->url) }}">{{ $menu->name }}</a>
                        </section>
                    @endforeach
